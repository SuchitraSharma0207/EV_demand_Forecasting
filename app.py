    ...
    for cty in multi_counties:
        ...
        for i in range(1, forecast_horizon + 1):
            ...
            if len(hist_ev) >= 3:
                ...
                print("Not enough data in hist_ev to extract lag features.")
                # You can either exit, raise an error, or use default/fallback values
                lag1, lag2, lag3 = 0, 0, 0  # example fallback
            roll_mean = np.mean([lag1, lag2, lag3])
            pct_change_1 = (lag1 - lag2) / lag2 if lag2 != 0 else 0
            pct_change_3 = (lag1 - lag3) / lag3 if lag3 != 0 else 0
            recent_cum = cum_ev[-6:]
